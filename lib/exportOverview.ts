...
export function addOverviewSheet(
  ...
  // Floating chart images don't create real rows, so pad blank rows up to the
  // intended position before adding the table — otherwise it could render
  // starting mid-image since addRow() only appends after the last real row.
